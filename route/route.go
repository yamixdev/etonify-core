...
func (r *Router) routeConnection(ctx context.Context, conn net.Conn, metadata adapter.InboundContext, onClose N.CloseHandlerFunc) error {
	...
	onClose, err = registerInterrupt(chain, N.NetworkTCP, conn, onClose)
	if err != nil {
		conn.Close()
		return err
	}
	leaf := chain[len(chain)-1]
	if outboundHandler, isHandler := leaf.(adapter.ConnectionHandler); isHandler {
		outboundHandler.NewConnection(ctx, conn, metadata, onClose)
	} else {
		r.connection.NewConnection(ctx, leaf, conn, metadata, onClose)
	}
	return nil
}
...
func (r *Router) routePacketConnection(ctx context.Context, conn N.PacketConn, metadata adapter.InboundContext, onClose N.CloseHandlerFunc) error {
	...
	ctx = interrupt.ContextWithIsExternalConnection(ctx)
	onClose, err = registerInterrupt(chain, N.NetworkUDP, conn, onClose)
	if err != nil {
		conn.Close()
		return err
	}
	leaf := chain[len(chain)-1]
	if outboundHandler, isHandler := leaf.(adapter.PacketConnectionHandler); isHandler {
		outboundHandler.NewPacketConnection(ctx, conn, metadata, onClose)
	} else {
		r.connection.NewPacketConnection(ctx, leaf, conn, metadata, onClose)
	}
	return nil
}
...
func (r *Router) preMatchFlow(ctx context.Context, metadata *adapter.InboundContext, packetDestination M.Socksaddr, matchedRule adapter.Rule, outboundTag string) adapter.PreMatchResult {
	...
	result.NewTracker = func() tun.FlowTracker {
		...
		if flowInterrupter := newFlowInterrupter(chain, metadataCopy.Network); flowInterrupter != nil {
